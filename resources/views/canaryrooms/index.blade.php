...
<article class="post">
  <header>
    <div class="title">
      <h2><a href="single.html">Canary Wharf Group Room Finder</a></h2>
      <p>Total:  {{ $canaryrooms->count() }} locations currently logged</p>
    </div>
    <div class="meta">
...
<form method="post" class="form-inline" action="canaryrooms/search" >
    {{ csrf_field() }}
...
<form method="post" class="form-inline" action="canaryrooms/searchsid" >
    {{ csrf_field() }}
  ...
  <button type="submit" class="btn btn-primary form-control mr-3 mb-2 mb-sm-3">Search</button>
     ...
     <div class="box alt">
      ...
      @foreach ($canaryroom as $C)




<div class="col-3-xlarge col-12-small"><a href="canaryrooms/{{ $C->id }}"><span class="image fit"><img src="/canaryimg/{{ $C->img1 }}" alt="" /></span> <p>{{ $C->sid }}</p> <h4>{{ $C->description }}</h4>  </a></div>
